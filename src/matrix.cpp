 ...
 * @exception std::invalid_argument Если размеры не положительные
 ...
 * @exception std::invalid_argument Если матрицы несовместимы по размерам
 */
...
/**
 * @brief Добавление скаляра к каждому элементу матрицы
 * @param m Исходная матрица
 * @param value Скалярное значение для добавления
 * @return Matrix Новая матрица с результатом операции
 * @note Каждый элемент результирующей матрицы: result[i][j] = m[i][j] + value
 */
Matrix matrix_add_scalar(const Matrix& m, double value) {
...
void print_matrix(const Matrix& m) {
    ...
    // 5. Информация о размере если матрица большая
    if (show_summary) {
